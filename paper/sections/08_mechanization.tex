...
The minimized positive bridge admits a second formulation. Lean defines \(R_{F,adm}^+\) as forward positivity rigidity restricted to admissible rigid extensions and proves
\[
\begin{array}{rcl}
R_{F,adm}^++G_F\text{-realization}&\Longrightarrow&Refl_G^+,\\
R_{F,adm}^++G_F\text{-realization}+Sym(R)&\Longrightarrow&Pers_G^+.
\end{array}
\]
Consequently \(R_{F,adm}^++Sym(R)\) can replace primitive \(STAB_G^+\) in the positive de-dicto theorem. The finite oracle checks this bridge exhaustively over 256 two-world, one-entity configurations in the classical negation-closed admissible fragment and retains countermodels when either symmetry or admissible rigidity is omitted.
